...
# for cifs flexvolume driver
sudo apt-get install -y cifs-utils

# to provide /dev/kmsg for kubelet
ln -s /dev/console /dev/kmsg
echo 'L /dev/kmsg - - - - /dev/console' > /etc/tmpfiles.d/kmsg.conf

# to workaround systemd read-write loop on kmsg/console
mkdir /etc/systemd/journald.conf.d/; echo "ReadKMsg=no" > /etc/systemd/journald.conf.d/kfilter.conf
systemctl restart systemd-journald

# may be needed for node-exporter to run on the node
# mount --make-rshared /
# echo '#!/bin/sh -e
